// MetallicPaint — iridescent metallic surface animation from React Bits (open source).
// Source: reactbits.dev/animations/metallic-paint
// Modified for frame use: canvas fills its container via ResizeObserver (not fixed 1000×1000),
// and imageSrc is optional — a synthetic radial gradient is auto-generated when omitted.
...
// Generate a flat synthetic ImageData when no imageSrc is provided.
// Creates a small texture at the container's aspect ratio so u_imgRatio ≈ u_ratio
// and the metallic effect fills the full canvas width.
// Skips the expensive SOR depth pass — uses a flat mid-gray depth instead.
function makeSyntheticImageData(containerW, containerH) {
  // Keep texture small (max 200px tall) to avoid heavy memory allocation
  const h = Math.max(1, Math.min(200, Math.round(containerH)));
  const w = Math.max(1, Math.round(h * (containerW / containerH)));
  const out = new ImageData(w, h);
  for (let i = 0; i < w * h; i++) {
    // Mid-gray depth (0.533) + fully opaque — treated as shape by the shader
    out.data[i * 4]     = 136;
    out.data[i * 4 + 1] = 136;
    out.data[i * 4 + 2] = 136;
    out.data[i * 4 + 3] = 255;
...
export default function MetallicPaint({
  ...
  const initGL = useCallback(() => {
    ...
    // alpha: false prevents the browser from treating canvas pixels as pre-multiplied
    // alpha=0, which makes the canvas render as fully transparent even with opaque content
    ...
    // Dark clear color matches the frame background so undrawn edges don't flash white
    gl.clearColor(0.063, 0.063, 0.063, 1.0);
  ...
  // ── Responsive canvas sizing — fills parent via ResizeObserver
  useEffect(() => {
    if (!initGL()) return;
    ...
    const updateSize = () => {
      ...
      // Keep shader aware of the container's aspect ratio
  ...
  // ── Load image (or generate synthetic texture) → process → upload
  useEffect(() => {
    if (!ready) return;
    if (!imageSrc) {
      // No source image: upload a flat depth map at the container's aspect ratio immediately.
      // This avoids the slow SOR pass and ensures the effect fills the full canvas width.
      const canvas = canvasRef.current;
      const parent = canvas?.parentElement;
      const w = Math.max(parent?.clientWidth || 100, 1);
      const h = Math.max(parent?.clientHeight || 100, 1);
      uploadTexture(makeSyntheticImageData(w, h));
